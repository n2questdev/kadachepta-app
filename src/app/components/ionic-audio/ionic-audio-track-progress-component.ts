...
@Component({
  ...
  template: `
    <time *ngIf="progress"><span *ngIf="audioTrack" [style.opacity]="audioTrack.duration > 0 ? 1 : 0">{{audioTrack.progress | audioTime}}</span></time>
    ...
    <time *ngIf="duration"><span *ngIf="audioTrack" [style.opacity]="audioTrack.duration > 0 ? 1 : 0">{{audioTrack.duration | audioTime}}</span></time>
    `
})
